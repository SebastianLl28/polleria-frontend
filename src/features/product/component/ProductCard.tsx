...
const ProductCard = ({
  ...
  const handleClick = () => {
    // addItem({ id, name, cardImage, description, price, valoration })
  ...
  return (
    ...
    >
      <CardHeader className='p-0'>
        <img src={cardImage} alt={name} className='h-48 object-cover' />
      </CardHeader>
      ...
      <Badge className='absolute right-4 top-4 space-x-1 text-base' variant={'secondary'}>
        ...
        <span>{new Decimal(valoration || 0).toFixed(0).toString()}</span>
        <Star size={18} />
      </Badge>
    </Card>
  )
